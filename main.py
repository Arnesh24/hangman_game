...
def get_word():
    ...
    for line in categories_text.split('\n'):
        ...
        time.sleep(1)
    while True:
        rule = input("Would you like to know the rules of the game(y/n):")
        if rule == 'y':
            print("Rules are as follows:")
            print("The game will have 6 tries before u lose, within those 6 tries you have to try and guess the word \n"
                  "There are 2 difficulty levels: easy and hard. There are no hints in easy mode, hard mode offers a hint")
        elif rule == 'n':
            print("Starting game!")
        else:
        ...
        while True:
            level_input = input("Enter the difficulty level (easy, hard): ").lower()
